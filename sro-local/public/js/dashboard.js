...
// ============ WEEK CALCULATION ============
function getWeekDisplay(patient) {
    // If no surgery date and no conservative care start, show 0
    if (!patient.surgery_date) {
        // Check if patient has conservative care data (episode_stage)
        // For now, if no surgery date, show "Week 0"
        return "Week 0 Treatment";
    }
    ...
    if (daysDiff < 0) {
        // Surgery is in the future — show weeks of conservative treatment
        // Count from episode creation or conservative care start
        // For now show "Pre-Op"
        return "Pre-Op";
    }
    
    // Post-op week calculation
    const fullWeeks = Math.floor(daysDiff / 7);
    const remainingDays = daysDiff % 7;
    
    // First week: before day 6, show "Week 0"
    if (fullWeeks === 0 && remainingDays < 6) {
        return "Week 0 Post-Op";
    }
    
    // Round: 5 or fewer remaining days = round down, 6 or 7 = round up
    const displayWeeks = remainingDays <= 5 ? fullWeeks : fullWeeks + 1;
    
    return `Week ${displayWeeks} Post-Op`;
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Restore surgeon filter from session (persists across pages)
    const savedSurgeonId = sessionStorage.getItem('selectedSurgeonId');
    if (savedSurgeonId) {
        const filterSurgeon = document.getElementById('filterSurgeon');
        if (filterSurgeon) {
            for (let opt of filterSurgeon.options) {
                if (opt.value === savedSurgeonId) {
                    filterSurgeon.value = savedSurgeonId;
                    filterPatients();
                    break;
                }
            }
    ...
    // Auto-open patient if ?openPatient= is in URL (returning from sub-page)
    const urlParams = new URLSearchParams(window.location.search);
    const autoOpenPatientId = urlParams.get('openPatient');
    if (autoOpenPatientId) {
        // Wait a tick for patients to be available, then open detail
        setTimeout(function() {
            if (patients.find(p => p.id === autoOpenPatientId)) {
                openPatientModal(autoOpenPatientId);
            }
        }, 500);
    }
});
...
function renderTierDecisionCard(item) {
    ...
    return '<div class="tier-item" onclick="openPatientModal(\'' + item.patientId + '\')">' +
        ...
        '<button class="tier-btn primary" onclick="event.stopPropagation();window.location.href=\'/cockpit-view.html?patient=' + item.patientId + '\'">🎛 Cockpit</button>' +
        smartBtn +
...
function renderPatientTable() {
    ...
    let filtered = patients.filter(p => {
        ...
        // Handle urgent filter separately - shows patients with alerts, high pain, or critical issues
        if (filterStatus === 'urgent') {
            const hasAlerts = (p.alert_count && p.alert_count > 0) || 
                             (p.last_pain_level && p.last_pain_level >= 7) ||
                             (p.last_pt_exercises === 0);
...
function filterPatients() {
    // Persist surgeon selection to sessionStorage
    var filterSurgeon = document.getElementById('filterSurgeon');
    if (filterSurgeon) {
        var val = filterSurgeon.value;
        if (val && val !== 'all') {
            sessionStorage.setItem('selectedSurgeonId', val);
            sessionStorage.setItem('selectedSurgeonName', filterSurgeon.options[filterSurgeon.selectedIndex].text);
        } else {
            sessionStorage.removeItem('selectedSurgeonId');
            sessionStorage.removeItem('selectedSurgeonName');
        }
    }
    loadStats();
    renderPatientTable();
...
async function openPatientModal(patientId) {
    ...
    // Portal button: visible only for nurse and admin roles
    const portalBtn = document.getElementById('portalBtnHeader');
    if (portalBtn) {
        portalBtn.style.display = (USER_ROLE === 'nurse' || USER_ROLE === 'admin') ? 'inline-block' : 'none';
    }
...
function renderSurgeonSummary(patientId) {
    ...
    if (patient?.surgery_date) {
        document.getElementById('sumDays').textContent = getWeekDisplay(patient);
    } else {
        document.getElementById('sumDays').textContent = 'Week 0 Treatment';
    }
...
function closePatientDetail() {
    ...
    // Hide Portal button
    var portalBtn = document.getElementById('portalBtnHeader');
