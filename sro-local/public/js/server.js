...
if (!epColsStage.includes('stage_data')) {
  // JSON object tracking per-stage completion: { "intake": { "completed_at": "...", "completed_by": "..." }, ... }
...
const EPISODE_STAGES = [
  {
    ...
    label: 'Intake',
    icon: '📋',
    description: 'Demographics, baseline PROMs, initial assessment',
    page: '/dashboard.html'  // patient modal on dashboard
  },
  {
    key: 'conservative_care',
    label: 'Conservative Care',
    icon: '💊',
    description: 'Non-operative treatment plan per AAOS guidelines',
    page: '/nonop-plan.html'
  },
  {
    key: 'preop_assessment',
    label: 'PreOp Assessment',
    icon: '🩺',
    description: 'Risk stratification, comorbidities, workup clearance',
    page: '/preop.html'
  },
  {
    key: 'surgical_prep',
    label: 'Surgical Prep',
    icon: '📝',
    description: 'Insurance, consent, disposition, prehab',
    page: '/surgical-prep.html'
  },
  {
    key: 'surgery',
    label: 'Surgery',
    ...
    page: null  // no dedicated page — marked by surgery_date
  },
  {
    key: 'postop',
    label: 'Post-Op',
    icon: '📊',
    description: 'Recovery tracking, check-ins, post-op PROMs',
    page: '/episode-timeline.html'
  }
];
...
// Check if advancing from current to target is valid (target must be next in sequence)
...
// Auto-detect stage completion based on existing data
function detectStageCompletion(episodeId, patientId, stageKey) {
  switch (stageKey) {
    case 'intake': {
      // Complete when: patient record exists AND (baseline PROM exists OR preop assessment exists)
      const hasProm = db.prepare(
        "SELECT id FROM pro_assessments WHERE patient_id = ? AND episode_id = ? LIMIT 1"
      ).get(patientId, episodeId);
      const hasPreop = db.prepare(
        "SELECT id FROM preop_assessments WHERE patient_id = ? LIMIT 1"
      ).get(patientId);
      return !!(hasProm || hasPreop);
    }
    case 'conservative_care': {
      // Complete when: explicitly marked (conservative care is variable per patient)
      // Some patients skip this if they've already failed conservative care elsewhere
      return false; // Always requires explicit completion
    }
    case 'preop_assessment': {
      // Complete when: preop assessment exists with status 'complete'
      const preop = db.prepare(
        "SELECT id FROM preop_assessments WHERE patient_id = ? AND (status = 'complete' OR status IS NULL) LIMIT 1"
      ).get(patientId);
      return !!preop;
    }
    case 'surgical_prep': {
      // Complete when: surgical_prep record has consent signed
      const prep = db.prepare(
        "SELECT id FROM surgical_prep WHERE patient_id = ? AND consent_signed_at IS NOT NULL LIMIT 1"
      ).get(patientId);
      return !!prep;
    }
    case 'surgery': {
      // Complete when: surgery_date is in the past
      const ep = db.prepare("SELECT surgery_date FROM episodes WHERE id = ?").get(episodeId);
      if (ep && ep.surgery_date) {
        return new Date(ep.surgery_date) <= new Date();
      }
      return false;
    }
    case 'postop': {
      // Complete when: 1-year PROM collected (episode effectively closed)
      const yearProm = db.prepare(
        "SELECT id FROM prom_schedule WHERE episode_id = ? AND window_name = '1_year' AND status = 'completed' LIMIT 1"
      ).get(episodeId);
      return !!yearProm;
    }
    default:
...
// GET stage definitions (so frontend knows labels, icons, order)
app.get('/api/stage-definitions', (req, res) => {
  res.json(EPISODE_STAGES);
});

// GET episode stage info
app.get('/api/episodes/:id/stage', (req, res) => {
  ...
  const stages = EPISODE_STAGES.map((s, idx) => {
    ...
    if (completed) {
      ...
      // Before current but not marked complete — treat as completed (stage was passed)
      status = 'completed';
    } else {
      status = 'upcoming';
    }
...
// POST advance to next stage (explicit completion of current stage)
app.post('/api/episodes/:id/advance-stage', (req, res) => {
  const { completed_by } = req.body;  // user ID or name who marked it complete

  const episode = db.prepare(
    'SELECT id, patient_id, episode_stage, stage_data FROM episodes WHERE id = ?'
  ).get(req.params.id);
  ...
  if (currentIdx >= STAGE_ORDER.length - 1) {
    return res.status(400).json({ error: 'Episode is already at final stage' });
  }
  ...
  db.prepare(`
    UPDATE episodes 
    SET episode_stage = ?, stage_data = ?, stage_updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
...
// POST set stage directly (clinician override — for cases like skipping conservative care)
app.post('/api/episodes/:id/set-stage', (req, res) => {
  const { stage, completed_by, reason } = req.body;

  if (!STAGE_ORDER.includes(stage)) {
    return res.status(400).json({ error: `Invalid stage: ${stage}. Valid: ${STAGE_ORDER.join(', ')}` });
  }
  ...
  for (let i = 0; i < targetIdx; i++) {
    ...
    if (!stageData[priorStage]) {
      stageData[priorStage] = {
        completed_at: new Date().toISOString(),
        completed_by: completed_by || null,
        skipped: i >= currentIdx,
        skip_reason: i >= currentIdx ? (reason || 'Clinician override') : undefined
      };
    }
  }
...
// POST mark current stage complete without advancing
// (Use case: all fields done, but don't want to move forward yet)
app.post('/api/episodes/:id/complete-stage', (req, res) => {
  const { completed_by } = req.body;

  const episode = db.prepare(
    'SELECT id, episode_stage, stage_data FROM episodes WHERE id = ?'
  ).get(req.params.id);
  ...
  db.prepare(`
    UPDATE episodes SET stage_data = ?, stage_updated_at = CURRENT_TIMESTAMP WHERE id = ?
